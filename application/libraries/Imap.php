...
class Imap{
    ...
    private $attachments_dir = './assets/attachments/';
    ...
    public function getFiles($r) { //save attachments to directory
		$pullPath = $this->attachments_dir . '/' . $r['uid'] . '_' . $r['file'];
		$res = true;
    ...
    private function loadMessage($uid, $type) {
        ...
        $array['date'] = date('M d, Y h:i:s A', strtotime($overview->date));
        $headers = $this->getHeaders($uid);
        $array['from'] = isset($headers->from) ? $this->processAddressObject($headers->from) : array('');
        $array['cc'] = isset($headers->cc) ? $headers->cc : array();
        $array['to'] = isset($headers->to) ? $headers->to : array();
        ...
        if ((isset($parameters['name']) || isset($parameters['filename'])) || (isset($structure->subtype) && strtolower($structure->subtype) == 'rfc822')
            ...
            if (!empty($parameters['charset']) && $parameters['charset'] !== 'UTF-8') {
                if (function_exists('mb_convert_encoding')) {
                    ...
                    // $messageBody = mb_convert_encoding($messageBody, 'UTF-8//TRANSLIT',$parameters['charset']);
                }
            }
    ...
    private function decode($text) {
        ...
        foreach (imap_mime_header_decode($text) as $word) {
            ...
            // $result .= mb_convert_encoding($word->text, 'utf-8',$ch);
        }
        return $result;
    }
    ...
    public function __destruct() {
        if (!empty($this->errors)) {
            foreach ($this->errors as $error) {
                error_log($error, 3, $base_url+'/imap-error.log');
            }
        }
    }
}
